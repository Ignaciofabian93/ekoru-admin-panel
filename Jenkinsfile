pipeline {
  ...
  environment {
    SERVICE_NAME = 'ekoru-admin-panel'
  }
  ...
  stages {
    ...
    stage('Deploy Staging') {
      ...
      steps {
        sh '''
          cp /opt/ekoru/secrets/ekoru-admin-panel/.env.staging ${WORKSPACE}/.env.staging
    ...
    stage('Deploy Production') {
      ...
      steps {
        sh '''
          cp /opt/ekoru/secrets/ekoru-admin-panel/.env.prod ${WORKSPACE}/.env.prod
